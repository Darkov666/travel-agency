...
const saveZone = () => {
    ...
    if (editingZone.value.isNew) {
        form.post(route('provider.zones.store'), {
            onSuccess: () => {
                closeModal(true);
            }
        });
    } else {
        form.put(route('provider.zones.update', form.id), {
            onSuccess: () => {
                closeModal(true);
            }
        });
...
const deleteZone = () => {
    ...
    form.delete(route('provider.zones.destroy', form.id), {
        onSuccess: () => {
...
watch(() => [props.zones, filterType.value], () => {
    loadZonesOnMap();
}, { deep: true });
...
<template>
    <Head title="Provider Zone Editor" />
        ...
        <template #header>
            <div class="flex justify-between items-center">
                ...
                <div class="text-sm text-gray-500">
                    Draw your operational zones on the map.
                </div>
            </div>
        </template>
